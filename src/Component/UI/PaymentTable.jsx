    ...
    <div className="w-full bg-white/70 backdrop-blur-md border border-slate-200/80 rounded-2xl sm:rounded-3xl p-3 sm:p-6 shadow-xl shadow-slate-200/50 mt-6 sm:mt-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 px-1 sm:px-2">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-base sm:text-lg font-bold text-slate-900 tracking-tight">Purchase History</h2>
            <p className="text-xs text-slate-500">View and manage all user recipe transactions</p>
          </div>
        </div>
              ...
              {purchases.map((purchase) => {
                return (
                  ...
                  >
                    ...
                    {/* Status Badge Cell */}
                    <Table.Cell className="py-3 sm:py-4 px-3 sm:px-6">
                      <span className="inline-flex items-center px-2 sm:px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-50 text-emerald-600 border border-emerald-200/60 whitespace-nowrap">
                        Completed
                      </span>
                    </Table.Cell>
                    ...
                    <Table.Cell className="py-3 sm:py-4 px-3 sm:px-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link href={`/recipes/${purchase.recipeId}`}>
                          <Button
                            size="sm"
                            className="bg-slate-100 hover:bg-orange-500 hover:text-white text-slate-700 font-semibold rounded-xl transition-all shadow-sm flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 text-xs whitespace-nowrap"
                          >
